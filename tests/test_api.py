...
def test_search_endpoint_success_hybrid():
    ...
    with patch('api.config', new={
        'LLM_MESH_MODEL': 'test-mesh-model',
        'EMBED_CONTENT_MODE': 'title_abstract',
        'NCBI_EMAIL': 'test@example.com',
        'OPENAI_EMBED_MODEL': 'test-embed-model'
    }) as mock_config, \
         patch('api.openai_client') as mock_openai, \
         patch('api.chroma_collection') as mock_collection, \
         patch('api.hybrid_rank_rrf') as mock_rrf, \
         patch('api.semantic_rank_articles') as mock_semantic, \
         patch('api.ensure_articles_in_db') as mock_ensure, \
         patch('api.search_pubmed_keywords') as mock_search_pubmed, \
         patch('api.generate_mesh_terms') as mock_generate_mesh:

        # --- Mock Configuration ---
        test_query = "test disease"
        mock_generate_mesh.return_value = ["Disease MeSH"]
        mock_search_pubmed.return_value = (['1', '2', '3'], "(test disease) AND (\"Disease MeSH\"[MeSH Major Topic])") # PMIDs and query used
        ...
        mock_collection.get.return_value = {
            'ids': ['2', '1', '3'],
            'documents': ['Title 2. Doc 2', 'Title 1. Doc 1', 'Title 3. Doc 3'], # Assume title+abstract embed mode
            'metadatas': [
                {
                    'title': 'Title 2', 'pub_date': '2023-01-01', 'journal': 'Journal B',
                    'authors': 'Doe J, Test T', 'mesh_headings': 'MeSH B, MeSH A',
                    'keywords': 'KW2, KW1', 'publication_types': 'Review', 'language': 'eng', 'doi': '10.doi/2'
                },
                {
                    'title': 'Title 1', 'pub_date': '2022-01-01', 'journal': 'Journal A',
                    'authors': 'Smith A', 'mesh_headings': 'MeSH A',
                    'keywords': 'KW1', 'publication_types': 'Journal Article', 'language': 'eng', 'doi': '10.doi/1'
                },
                {
                    'title': 'Title 3', 'pub_date': '2021-01-01', 'journal': 'Journal C',
                    'authors': 'Ray B', 'mesh_headings': 'MeSH C',
                    'keywords': '', 'publication_types': 'Clinical Trial', 'language': 'fre', 'doi': '10.doi/3'
                }
            ]
        }

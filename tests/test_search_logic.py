...
from search import (Author, PubMedArticle, ensure_articles_in_db,
                    fetch_abstracts, generate_mesh_terms,
                    get_openai_embeddings, hybrid_rank_rrf,
                    search_pubmed_keywords, semantic_rank_articles)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"Year": "2023", "Month": "13"},
            "2023-01-01",
        ),  # Invalid month defaults to Jan
        (
            {"Year": "2023", "Month": "Apr", "Day": "31"},
            "2023-04-30",
        ),  # Invalid day clamped
    ],
)
...
def test_get_openai_embeddings_success(mock_openai_client_embed):
    ...
    mock_openai_client_embed.embeddings.create.assert_called_once_with(
        input=texts, model=model
    )


def test_get_openai_embeddings_api_error(mock_openai_client_embed):
    mock_openai_client_embed.embeddings.create.side_effect = Exception(
        "Embed API Error"
    )
...
def test_search_pubmed_keywords_mesh_major_success(mock_esearch, mock_read):
    ...
    mock_esearch.assert_called_once_with(
        db="pubmed", term=expected_query, retmax=str(max_results), sort="relevance"
    )
    mock_read.assert_called_once_with(
        mock_handle
    )  # Check read was called with the mock handle
...
def test_search_pubmed_keywords_fallback_to_mesh_terms(mock_esearch, mock_read):
    ...
    expected_query_attempt2 = (
        '(original query text) AND ("TermA"[MeSH Terms] OR "TermB"[MeSH Terms])'
    )
...
def test_search_pubmed_keywords_no_mesh(mock_esearch):
    ...
    with patch("search.Entrez.read", mock_read_instance):
        ...
        pmids, query_used = search_pubmed_keywords(
            query, mesh_terms, max_results, email
        )
...
def test_hybrid_rank_rrf_basic():
    ...
    assert (
        rrf_results[0][1] > rrf_results[2][1]
    )  # Score of 1/2 should be > score of 3/4
...
def test_hybrid_rank_rrf_disjoint():
    ...
    semantic_ranked = [
        ("2", 0.9),
        ("3", 0.8),
    ]  # PMID 1 only in pubmed, 3 only in semantic
    k = 1
...
def test_semantic_rank_articles_success(mock_get_embed, mock_chroma_collection_query):
    ...
    expected_ranking = [
        ("pmid1", pytest.approx(0.9)),
        ("pmid2", pytest.approx(0.8)),
        ("pmid3", pytest.approx(0.7)),
    ]

    ranked_results = semantic_rank_articles(
        query,
        pmids_to_rank,
        mock_chroma_collection_query,
        top_k,
        mock_openai_client,
        embed_model,
    )
...
@patch("search.get_openai_embeddings")
def test_semantic_rank_articles_empty_input(
    mock_get_embed, mock_chroma_collection_query
):
...
@patch("search.get_openai_embeddings")
def test_semantic_rank_articles_embed_fail(
    mock_get_embed, mock_chroma_collection_query
):
...
@patch("search.get_openai_embeddings")
def test_semantic_rank_articles_chroma_fail(
    mock_get_embed, mock_chroma_collection_query
):
...
@patch("search.get_openai_embeddings")
def test_semantic_rank_articles_top_k_limit(
    mock_get_embed, mock_chroma_collection_query
):
...
@patch("search.get_openai_embeddings")
def test_semantic_rank_articles_filters_pmids(
    mock_get_embed, mock_chroma_collection_query
):
...
def test_fetch_abstracts_no_abstract_skipped(mock_efetch, mock_read):
    ...
    combined_records = {
        "PubmedArticle": SAMPLE_EFETCH_RECORD_MINIMAL["PubmedArticle"]
...
@patch("search.get_openai_embeddings")
def test_ensure_articles_all_exist(
    mock_get_embed, mock_fetch, mock_chroma_collection_ensure
):
    """Test case where all requested PMIDs already exist in DB."""
    pmids_to_check = ["1", "2", "3"]
    mock_chroma_collection_ensure.get.return_value = {
        "ids": ["1", "2", "3"]
    }  # Simulate all exist
    mock_openai_client = MagicMock()

    result = ensure_articles_in_db(
        pmids_to_check, mock_chroma_collection_ensure, mock_openai_client, "m", "a", "e"
    )

    assert result == ["1", "2", "3"]  # Should return the original list
    mock_chroma_collection_ensure.get.assert_called_once_with(
        ids=pmids_to_check, include=[]
    )
...
@patch("search.fetch_abstracts")
@patch("search.get_openai_embeddings")
def test_ensure_articles_none_exist(
    mock_get_embed,
    mock_fetch,
    mock_chroma_collection_ensure,
    mock_fetched_articles,
    mock_embeddings_ensure,
):
...
@patch("search.fetch_abstracts")
@patch("search.get_openai_embeddings")
def test_ensure_articles_some_exist(
    mock_get_embed,
    mock_fetch,
    mock_chroma_collection_ensure,
    mock_fetched_articles,
...
@patch("search.fetch_abstracts")
@patch("search.get_openai_embeddings")
def test_ensure_articles_fetch_fails(
    mock_get_embed, mock_fetch, mock_chroma_collection_ensure
):
...
@patch("search.fetch_abstracts")
@patch("search.get_openai_embeddings")
def test_ensure_articles_embed_fails(
    mock_get_embed, mock_fetch, mock_chroma_collection_ensure, mock_fetched_articles
):
...
@patch("search.fetch_abstracts")
@patch("search.get_openai_embeddings")
def test_ensure_articles_add_fails(
    mock_get_embed,
    mock_fetch,
    mock_chroma_collection_ensure,
    mock_fetched_articles,
    ...
    mock_chroma_collection_ensure.add.side_effect = Exception(
        "Add failed"

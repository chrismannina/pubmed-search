...
from search import OPENAI_EMBED_MODEL  # Need the specific embedding model name
from search import DEFAULT_EMBED_CONTENT_MODE, DEFAULT_LLM_MESH_MODEL
from search import Author as LogicAuthor
from search import PubMedArticle as LogicPubMedArticle
from search import (ensure_articles_in_db, generate_mesh_terms,
                    hybrid_rank_rrf, search_pubmed_keywords,
...
class SearchRequest(BaseModel):
    ...
    ranking_mode: Literal["pubmed", "semantic", "hybrid"] = Field(
        default="hybrid", description="The ranking strategy to use."
    )
    min_year: Optional[int] = Field(
        default=None,
        description="Optional minimum publication year to filter results before ranking.",
    )
    top_k: int = Field(
        default=10, gt=0, le=50, description="Number of top results to return."
    )
    max_pubmed_results: int = Field(
        default=100,
        gt=0,
        le=500,
        description="Maximum results from initial PubMed search.",
    )
    # Allow overriding certain configs per request
    llm_mesh_model: Optional[str] = Field(
        default=None, description="Override LLM model for MeSH generation."
    )
    embed_content_mode: Optional[Literal["abstract", "title_abstract"]] = Field(
        default=None, description="Override content embedding mode."
    )
...
async def perform_search(request: SearchRequest):
    ...
    print(
        f"Received search request: Query='{request.query}', Mode='{request.ranking_mode}', Year>='{request.min_year}', K='{request.top_k}'"
    )
    ...
    generated_mesh_terms = generate_mesh_terms(
        request.query, openai_client, model=llm_mesh_model
    )
        ...
        try:
            ...
            if filter_data and filter_data.get("ids"):
                for i, pmid in enumerate(filter_data["ids"]):
                    meta = (
                        filter_data["metadatas"][i]
                        if i < len(filter_data.get("metadatas", []))
                        else {}
                    )
                    metadata_for_filtering[pmid] = meta
        except Exception as e:
            print(
                f"Warning: Could not retrieve metadata for date filtering: {e}. Filter skipped."
            )
    ...
    if not eligible_pmids_for_ranking:
        return SearchResponse(
            ...
            pubmed_query_used=pubmed_query_used,
            results_count=0,
            results=[],  # No results after date filter
        )

    # Filter the original ordered list based on eligibility
    pubmed_ranked_eligible_pmids = [
        pmid for pmid in initial_pmids_ordered if pmid in eligible_pmids_for_ranking
    ]
    ...
    if request.ranking_mode == "pubmed":
        print(
            f"Using PubMed relevance ranking on {len(pubmed_ranked_eligible_pmids)} eligible articles..."
        )
        final_ranked_pmid_scores = [
            (pmid, i + 1.0)
            for i, pmid in enumerate(pubmed_ranked_eligible_pmids[: request.top_k])
        ]

    elif request.ranking_mode == "semantic":
        print(
            f"Performing semantic ranking on {len(eligible_pmids_for_ranking)} eligible articles..."
        )
        final_ranked_pmid_scores = semantic_rank_articles(
            request.query,
            eligible_pmids_for_ranking,
            chroma_collection,
            request.top_k,
            openai_client,
            embed_model,
        ...
        print(
            f"Performing hybrid ranking (RRF) on {len(eligible_pmids_for_ranking)} eligible articles..."
        )
        print(
            f"  Getting semantic scores for {len(eligible_pmids_for_ranking)} articles..."
        )
        all_semantic_results = semantic_rank_articles(
            request.query,
            eligible_pmids_for_ranking,
            chroma_collection,
            len(eligible_pmids_for_ranking),
            openai_client,
            embed_model,
        )
        if not all_semantic_results:
            print(
                "  Warning: Failed to get semantic scores for RRF. Falling back to PubMed ranking."
            )
            final_ranked_pmid_scores = [
                (pmid, i + 1.0)
                for i, pmid in enumerate(pubmed_ranked_eligible_pmids[: request.top_k])
            ...
            rrf_results = hybrid_rank_rrf(
                pubmed_ranked_eligible_pmids, all_semantic_results
            )
    ...
    if final_pmids:
        try:
            top_articles_data = chroma_collection.get(
                ids=final_pmids, include=["documents", "metadatas"]
            )
            if top_articles_data and top_articles_data.get("ids"):
                ...
                for i, pmid_retrieved in enumerate(top_articles_data["ids"]):
                    ...
                    document = (
                        retrieved_documents[i] if i < len(retrieved_documents) else ""
                    )
                    display_data[pmid_retrieved] = {
                        "metadata": metadata,
                        "document": document,
                    }
        except Exception as e:
    ...
    for rank, (pmid, score) in enumerate(final_ranked_pmid_scores):
        ...
        item = SearchResultItem(
            ...
            publicationTypes=meta.get("publication_types", "").split(", ")
            if meta.get("publication_types")
            else [],
            meshHeadings=meta.get("mesh_headings", "").split(", ")
            if meta.get("mesh_headings")
            else [],

...
def generate_mesh_terms(
    query: str, llm_client: OpenAI, model: str = DEFAULT_LLM_MESH_MODEL
) -> list[str]:
    ...
    try:
        response = llm_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert medical librarian specializing in MeSH terms.",
                },
...
def search_pubmed_keywords(
    query: str, mesh_terms: list[str] | None, max_results: int, entrez_email: str
) -> tuple[list[str], str]:
    ...
    if mesh_terms:
        major_topic_query_part = " OR ".join(
            [f'"{term}"[MeSH Major Topic]' for term in mesh_terms]
        )
        if major_topic_query_part:
            refined_query = f"({query}) AND ({major_topic_query_part})"
            print(
                f"Executing PubMed query (Attempt 1: Text AND MeSH Major Topics): {refined_query}"
            )
            try:
                ...
                if len(pmids) >= min_results_threshold:
                    ...
                    print(
                        f"Attempt 1 yielded only {len(pmids)} results (threshold: {min_results_threshold}). Trying broader MeSH Terms..."
                    )
            except Exception as e:
                print(
                    f"Error during PubMed search (Attempt 1): {e}. Trying broader MeSH Terms..."
                )
                # Proceed to Attempt 2
        else:
            print(
                "Warning: MeSH terms provided but resulted in an empty Major Topic query part. Skipping Attempt 1."
            )
    ...
    if mesh_terms and (
        not pmids or len(pmids) < min_results_threshold
    ):  # Check if we need to run this attempt
        ...
        if terms_query_part:
            ...
            if refined_query_broad != final_query_used:
                print(
                    f"Executing PubMed query (Attempt 2: Text AND MeSH Terms): {refined_query_broad}"
                )
                try:
                    handle = Entrez.esearch(
                        db="pubmed",
                        term=refined_query_broad,
                        retmax=str(max_results),
                        sort="relevance",
                    )
                    record = Entrez.read(handle)
                    ...
                    if len(pmids) >= min_results_threshold:
                        ...
                        print(
                            f"Attempt 2 yielded only {len(pmids)} results. Falling back to original query text..."
                        )
                except Exception as e:
                    print(
                        f"Error during PubMed search (Attempt 2): {e}. Falling back to original query text..."
                    )
                    # Proceed to Attempt 3
            else:
                print(
                    "Broad MeSH query is same as a previous attempt. Falling back to original query text..."
                )
        else:
            print(
                "Warning: MeSH terms provided but resulted in an empty MeSH Terms query part. Falling back to original query text."
            )
    ...
    if (
        not pmids or len(pmids) < min_results_threshold
    ):  # Check if we need to run this fallback
        # Avoid re-running if original query was already tried and failed
        if (
            final_query_used != query or not pmids
        ):  # Check if original query differs or pmids is empty
            ...
            try:
                ...
                print(
                    f"Found {len(pmids)} potential PMIDs using only original query text."
                )
            except Exception as e:
            ...
            print(
                "Original query already attempted or results were insufficient. Returning current results."
            )
...
def parse_pubmed_date(date_info: dict) -> str | None:
    ...
    try:
        ...
        if month_str in month_map:
            ...
            if not 1 <= month <= 12:
                print(
                    f"  Warning: Invalid month value '{month_str}' parsed. Defaulting month to 1."
                )
                month = 1  # Reset to default if invalid
        # else: month remains 1 (default)

        day = (
            int(day_str) if day_str and day_str.isdigit() else 1
        )  # Default to 1st if day is missing/invalid
...
def fetch_abstracts(pmids: list[str], entrez_email: str) -> Dict[str, PubMedArticle]:
    ...
    for i in range(0, len(pmids), batch_size):
        ...
        try:
            handle = Entrez.efetch(
                db="pubmed", id=batch_pmids, rettype="medline", retmode="xml"
            )
            ...
            for record in records.get("PubmedArticle", []):
                try:
                    ...
                    language = article_info.get("Language", [None])[
                        0
                    ]  # Takes first language
                    pagination = article_info.get("Pagination", {}).get(
                        "MedlinePgn", None
                    )

                    # --- Abstract ---
                    abstract = None
                    abstract_list = article_info.get("Abstract", {}).get(
                        "AbstractText", []
                    )
                    if isinstance(abstract_list, list):
                        abstract = " ".join(
                            str(a) for a in abstract_list
                        )  # Ensure all parts are strings
                    elif (
                        abstract_list
                    ):  # Handle case where it might be a single string directly
                        abstract = str(abstract_list)
                    if not abstract:
                        other_abstract_list = article_info.get("OtherAbstract", [])
                        if (
                            other_abstract_list
                            and isinstance(other_abstract_list, list)
                            and len(other_abstract_list) > 0
                        ):
                            abstract_texts = other_abstract_list[0].get(
                                "AbstractText", []
                            )
                    ...
                    keywords = []
                    if (
                        isinstance(keyword_list_data, list)
                        and len(keyword_list_data) > 0
                    ):
                    ...
                    if isinstance(article_ids, list):
                        for article_id_obj in article_ids:
                            ...
                            if (
                                isinstance(article_id_obj, dict)
                                and article_id_obj.get("IdType") == "doi"
                            ):
                            ...
                            # Fallback check if it acts like an object with attributes (less likely based on error)
                                ...
                                hasattr(article_id_obj, "attributes")
                                and article_id_obj.attributes.get("IdType") == "doi"
                            ):
                                doi = str(
                                    article_id_obj
                                )  # Keep original string conversion as last resort
                                break
    ...
    print(
        f"Successfully processed details for {len(articles)} articles with abstracts."
    )
    return articles


def get_openai_embeddings(
    texts: list[str], openai_client: OpenAI, model: str
) -> list[list[float]]:
...
def ensure_articles_in_db(
    pmids_to_check: list[str],
    collection: chromadb.Collection,
    openai_client: OpenAI,
    embed_model: str,
    embed_content_mode: str,
    entrez_email: str,
) -> list[str]:
    ...
    if pmids_to_fetch:
        ...
        if new_articles_data:
            ...
            if len(valid_indices) < len(new_pmids):
                print(
                    f"  Warning: Skipping {len(new_pmids) - len(valid_indices)} articles with empty content for embedding."
                )
            ...
            if new_pmids:  # Proceed only if we have valid articles
                ...
                for pmid in new_pmids:
                    ...
                    meta = {
                        ...
                        "authors": ", ".join(
                            author.display_name() for author in article_model.authors
                        ),
                ...
                new_embeddings = get_openai_embeddings(
                    documents, openai_client, embed_model
                )
                ...
                if new_embeddings and len(new_embeddings) == len(new_pmids):
                    ...
                    print(
                        "Failed to generate embeddings for new articles. They won't be added."
                    )
    ...
    final_available_pmids = [
        pmid for pmid in pmids_to_check if pmid in confirmed_available_ids
    ]
    print(
        f"{len(final_available_pmids)} PMIDs are now available in ChromaDB for ranking."
    )
...
def semantic_rank_articles(
    ...
    print(
        f"Querying ChromaDB (requesting up to {num_results_to_request_query}) to return top {num_results_to_return}..."
    )
    try:
...
def hybrid_rank_rrf(
    pubmed_ranked_pmids: list[str],
    semantic_ranked_results: list[tuple[str, float]],
    k: int = RRF_K,
) -> list[tuple[str, float]]:
    ...
    semantic_ranks = {
        pmid: rank + 1 for rank, (pmid, _) in enumerate(semantic_ranked_results)
    }
